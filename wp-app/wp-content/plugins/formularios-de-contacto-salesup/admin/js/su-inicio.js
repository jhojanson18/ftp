var _inicio = function(){
	...
	var $mainContener = $("#suMainContent");
	...
	this.modificaIntegracion = function(tk){
		...
		//$("#suToken").val(objTokenIntegracion.token);
		$('#modalModifica').modal({backdrop: 'static', keyboard: false}).modal('show');
		$(".obligatorio").removeClass('is-invalid');
	}

	this.guardaIntegracion = function(id){
		$("#btnActualizaIntegracion").attr('disabled',true);
		SAUP190122fn.loader();
		var idIntegracion = id;
		var suToken = $('#suToken').val().trim();
		...
		if(SAUP190122fn.validaObligatorio() && sesion!='error'){
			var procesaGuardado = function(res, err){
				if(err != 'success' && err){
					SAUP190122fn.loaderElimina();
					SAUP190122fn.mensaje('error');
					$("#btnActualizaIntegracion").attr('disabled',false);
 					return false;
				}
